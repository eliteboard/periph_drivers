...
int8_t int_dac_arm(struct int_dac_dev_s *self)
{
    ...
    if(HAL_DAC_Start_DMA(self->hdac, DAC_CHANNEL_1, int_dac1_dma_buffer, INT_DAC_BUFFER_LENGTH, DAC_ALIGN_12B_R) != HAL_OK)
    {
        /* Start Error */
        Error_Handler();
    }
    // if(HAL_DAC_Start_DMA(self->hdac, DAC_CHANNEL_2, int_dac2_dma_buffer, INT_DAC_BUFFER_LENGTH, DAC_ALIGN_12B_R) != HAL_OK)
    // {
    //     /* Start Error */
    //     Error_Handler();
    // }
